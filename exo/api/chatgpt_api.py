...
class ChatGPTAPI:
  def __init__(self, node: Node, inference_engine_classname: str, response_timeout: int = 90, on_chat_completion_request: Callable[[str, ChatCompletionRequest, str], None] = None):
    ...
    cors.add(self.app.router.add_get("/enginepool", self.handle_model_support), {"*": cors_options})
  ...
  async def handle_model_support(self, request):
    return web.json_response({"model pool": [k for k,v in model_cards.items() if all(list(dict.fromkeys([inference_engine_classes.get(i,None) for i in self.node.topology_inference_engines_pool for i in i if i is not None] + [self.inference_engine_classname])).map(lambda e: e in v["repo"]))]})
